...
# som glboal variables which should not be edited, they are used to keep track of the test sessions
LOG_FILE="tests.log"
TEST_ID=0
...
# yes imeans default configuration for simulation of message processing
...
#this is the list of paths to the topolgy config txt files.
topology_list=("../test/tests_topology/config.txt")

# this is the list of catalogs which will be accessed to get the elemets information for each topology file
			...
			do
				...
				do
					...
					do
						...
						BEGIN="BEGIN test $TEST_ID:.............$DATE"
						END="Test $TEST_ID COMPLETE"
						ERR_RETRY="MAX RETRY ($MAX_RETRY) reached for test $TEST_ID! SKIPPING"
						NUM_RETRIES=0


						topology_arg="--cat=$catalog --top=$topology"
						output="$(pwd)/$path/tests/$simulator_arg-$execution_arg-threads_$thread_num-$scheduler-preempt_$preemption_choice-sim_proc_$sim_processing_choice"

						out_arg="--out=$output"

						test_cmd="bash start.sh -q -g -a -s $topology_arg $simulator_arg $execution_arg $thread_arg $scheduler_arg  $preemption_arg $sim_processing_arg $out_arg"
...
DATE="$(date +%d)/$(date +%m)/$(date +%Y) - $(date +%H):$(date +%M):$(date +%S)"
echo "$END_SESSION AT $DATE" >> $LOG_FILE
echo "$END_SESSION AT $DATE"